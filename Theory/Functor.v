...
Section Functor_Setoid.
  ...
  Program Instance Functor_Setoid
    ...
    { (** The pointwise object equality makes the proofs too tedious since
        ...
        * the proofs quite verbose. Instead, we use isomorphisms between [F x] and
        * [G x] for each object [x : C], which is more flexible and convenient.
        *)
      equiv := λ F G, { iso : ∀ x : C, F x ≅ G x
                      & ∀ `(f : x ~> y), fmap[F] f ≡ (iso y)⁻¹ ∘ fmap[G] f ∘ (iso x)
                      }
